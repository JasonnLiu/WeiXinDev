...
import com.jason.WeiXinDev.dao.UserDao;
...
public class LBService {
	...
	public static NewsMessage process(Map<String, String> map) throws IOException {
		...
		UserDao.save(map.get("FromUserName"), Location_X, Location_Y, B_Location[0], B_Location[1]);
		...
		String url_replace = url_init.replace("loc_x", B_Location[0]).replace("loc_y", B_Location[1]).replace("AK", ak);
	...
	public static String process(Map<String, String> map, String keyword) {
		String[] B_Location = new String[2];
		// String loc_x = B_Location[0] ;
		// String loc_y = B_Location[1] ;

		String open_id = map.get("FromUserName");
		B_Location = UserDao.queryUserLoc(open_id);
		...
		if (B_Location == null) {
			...
			tm.setMsgType(MessageUtil.RESP_MESSAGE_TYPE_TEXT);
			tm.setCreateTime(new Date().getTime());
			String respContent = "请发送你的位置";
			logger.info("no function");
			tm.setContent(respContent);
			return MessageUtil.message2xml(tm);
		} else {

			String ak = CommUtil.getProperty("ak", "B_Map.properties");

			String url_replace = url_init.replace("loc_x", B_Location[0]).replace("loc_y", B_Location[1]).replace("AK",
					ak);
			String url_encoded = null;
			try {
				url_encoded = url_replace.replace("Query", URLEncoder.encode(keyword, "UTF-8"));
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			String respStr = httpRequest(url_encoded);
			...
			logger.info("创建NewsMessage....end");
			logger.info("LBService.process()....end");
			return MessageUtil.message2xml(nm);
		}
	}
